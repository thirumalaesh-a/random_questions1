    ...
    std::vector<std::vector<int>> bfs(std::vector<std::vector<int>>& grid, 
        ...
        while(!q.empty()) {
            ...
            for(auto [deltaRow, deltaCol] : directions) {
                ...
                // Walk on empty land (0) only
                if(newRow >= 0 && newRow < m && newCol >= 0 && newCol < n &&
                   distance[newRow][newCol] == -1 && grid[newRow][newCol] == 0) {
...
public:
     ...
     */
    void printDistanceMap(std::vector<std::vector<int>>& distanceMap, int numPoi){
        std::cout << "POI " << numPoi << " distances:\n";

        for(int row = 0; row < distanceMap.size(); row++){  
            for(int col = 0; col < distanceMap[row].size(); col++){
                if(distanceMap[row][col] == -1) std::cout << "  X";
                else std::cout << " " << distanceMap[row][col];
            }
            std::cout << "\n";
        }
    }
    int findOptimalSpot(std::vector<std::vector<int>>& grid) {
        ...
        for(auto [i, j] : parkingSpots) {
            allDistances.push_back(bfs(grid, i, j));
        }
        ...
        for(int i = 0; i < m; i++) {
            for(int j = 0; j < n; j++) {
                ...
                for(int k = 0; k < numSpots; k++) {
                    ...
                    reach++;
                    totalDist += allDistances[k][i][j];  // SUM aggregation
                }
...
int main() {
    ...
    standard.findOptimalSpot(grid);
    
    ParkingSystemOptimized optimized;
    optimized.findOptimalSpot(grid);
    
    std::cout << "\n=== COMPLEXITY COMPARISON ===\n";
    std::cout << "Standard:  Time O(k×m×n), Space O(k×m×n)\n";
    std::cout << "Optimized: Time O(k×m×n), Space O(m×n)\n";
    std::cout << "\nStandard advantage:  Supports Minimax/MaxMin/Weighted\n";
    std::cout << "Optimized advantage: 96%+ memory savings for large k\n";
